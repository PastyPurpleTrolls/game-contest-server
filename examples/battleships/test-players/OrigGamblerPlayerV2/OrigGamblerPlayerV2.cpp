...
OrigGamblerPlayerV2::OrigGamblerPlayerV2( int boardSize ):
    PlayerV2(boardSize)
{
    lastRow = 0;
    lastCol = 0;
 ...
 */
void OrigGamblerPlayerV2::initializeBoard(char board[MAX_BOARD_SIZE][MAX_BOARD_SIZE]) {
    for(int row=0; row<boardSize; row++) {
	for(int col=0; col<boardSize; col++) {
	    board[row][col] = WATER;
	}
 ...
 */
void OrigGamblerPlayerV2::newRound() {
 ...
 */
Message OrigGamblerPlayerV2::getMove() {
    getShot(shot);
 ...
 */
void OrigGamblerPlayerV2::update(Message msg) {
 ...
 */
Message OrigGamblerPlayerV2::placeShip(int length) {
    char shipName[10];
 ...
 */
void OrigGamblerPlayerV2::markShip( int row, int col, int length, Direction dir ) {
    if( dir==Horizontal ) {
 ...
 */
bool OrigGamblerPlayerV2::positionOk( int row, int col, int length, Direction dir ) {
    if( dir==Horizontal ) {
	for(int c=col; c<col+length; c++) {
...
void OrigGamblerPlayerV2::getShot(int shot[]) {
 ...
 */
void OrigGamblerPlayerV2::chooseValues( int& row, int& col, int& length, Direction& dir ) {
...
void OrigGamblerPlayerV2::getNextProbe( int shot[] ) {
    reWeighBoard( cellWeights );
...
// followup locations.
void OrigGamblerPlayerV2::getFollowUpShot( int row, int col, int shot[] ) {
...
bool OrigGamblerPlayerV2::search( int row, int col, int rowDelta, int colDelta, int shot[], int rangeLeft ) {
...
bool OrigGamblerPlayerV2::haveUnfinishedBusiness( int position[2] ) {
    for( int row=0; row<boardSize; row++ ) {
	for( int col=0; col<boardSize; col++ ) {
...
void OrigGamblerPlayerV2::reWeighBoard( int board[][MAX_BOARD_SIZE] ) {
    for( int row=0; row<boardSize; row++ ) {
	for( int col=0; col<boardSize; col++ ) {
...
void OrigGamblerPlayerV2::printWeightedBoard( int board[][MAX_BOARD_SIZE] ) {
    int value;
    for( int row=0; row<boardSize; row++ ) {
	for( int col=0; col<boardSize; col++ ) {
	    value = board[row][col];
...
void OrigGamblerPlayerV2::getMaxWeightCell( int board[][MAX_BOARD_SIZE], int shot[2] ) {
...
int OrigGamblerPlayerV2::calcCellWeight( int row, int col ) {
...
int OrigGamblerPlayerV2::calcCellWeightVert( int row, int col, int length ) {
    int weight = 0;
    char status;
...
// Assistant to calcCellWeight( int, int ) above.
int OrigGamblerPlayerV2::calcCellWeightHoriz( int row, int col, int length ) {
    int weight = 0;
    char status;
...
bool OrigGamblerPlayerV2::onBoard( int row, int col ) {
    return row>=0 && col>=0 && row<boardSize && col<boardSize; 
}

// Think C/C++ macro. Just to avoid typing the whole mess in
// every time!
char OrigGamblerPlayerV2::getCellStatus( int row, int col ) {
