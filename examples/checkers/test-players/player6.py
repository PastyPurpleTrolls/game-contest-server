...
def getMove(board,playerIndex,playerColors,playerTokens,opponentTokens,rowInc):
    ...
    if len(validJumps)==0:
        if kingOutMoves != []:#H5: get those kings out of that place where we make kings!
        ...
        elif movestogether != []:#Heuristic 4: move pieces next to each other
            move = movestogether[random.randrange(0,len(movestogether))]
        else:#Heuristic 3: move toward center of board
        ...
        else: #heuristic 1: find longest jump
