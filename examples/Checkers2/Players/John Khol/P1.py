...
def countEnemyCheckers(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    enemyCount=0
    for row in board:
        for space in row:
            if space in opponentTokens:
                enemyCount+=1
    return enemyCount

def countRow(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    row0=board[0].count(opponentTokens[0])+board[0].count(opponentTokens[1])    
    row1=board[1].count(opponentTokens[0])+board[1].count(opponentTokens[1])
    row2=board[2].count(opponentTokens[0])+board[2].count(opponentTokens[1])
    row3=board[3].count(opponentTokens[0])+board[3].count(opponentTokens[1])
    row4=board[4].count(opponentTokens[0])+board[4].count(opponentTokens[1])
    row5=board[5].count(opponentTokens[0])+board[5].count(opponentTokens[1])
    row6=board[6].count(opponentTokens[0])+board[6].count(opponentTokens[1])
    row7=board[7].count(opponentTokens[0])+board[7].count(opponentTokens[1])
    rowList=[row0,row1,row2,row3,row4,row5,row6,row7]
    theRow=rowList.index(max(rowList))
    return theRow
#*********************************************************************************
#***************************** Heuristics ****************************************
#*********************************************************************************
...
def movesIntoSafeSpot(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    ...
    return lst
#Heuristic 6
#checks for moves into side squares
def moveIntoSideSquare(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    lst=[]
    for mve in validMoves:
        if mve[-1] in ['0','7'] and ord(mve[0])-65 not in [0,7]:
            lst.append(mve)
    return lst

#Heuristic 7
#checks to see if move moves checker away from a jump
def moveFromJump(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    ...
    for mve in validMoves:
        for jmpe in enemyJumps:
            if between(jmpe,mve[0:2]):
                lst.append(mve)
    return lst

#Heuristic 8
#checks to see if moving that checker reduces the amount of enemy jumps in any way
def moveBlockJump(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    enemyIndex,enemySymbols,mySymbols,enemyRowInc=swapPlayer(playerIndex)
    testBoard=copy.deepcopy(board)
    lst=[]
    enemyJumps=listValidJumps(testBoard,enemyIndex,enemySymbols,mySymbols,enemyRowInc)
    jumpCount=len(enemyJumps)
    for mv in validMoves:
        fromRow=ord(mv[0:1])-65
        fromCol=int(mv[1:2])
        toRow=ord(mv[3:4])-65
        ...
        testBoard[toRow][toCol]=testBoard[fromRow][fromCol]
        testBoard[fromRow][fromCol]=0
        enemyJumps2=listValidJumps(testBoard,enemyIndex,enemySymbols,mySymbols,enemyRowInc)
        jumpCount2=len(enemyJumps2)
        if jumpCount2<jumpCount:
            lst.append(mv)
    return lst

#Heuristic 9
#checks to see if using that checker to jump another reduces the amount of enemy jumps in any way
...
#Heuristic 10
#checks to see if any available jumps jump a king
def jumpKingChecker(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    lst=[]
    testBoard=copy.deepcopy(board)
    for jmp in validJumps:
        fromRow=ord(jmp[0:1])-65
        ...
        toRow=ord(jmp[-2:-1])-65
        toCol=int(jmp[-1:])
        betRow=(fromRow+toRow)//2
        betCol=(fromCol+toCol)//2
        if testBoard[betRow][betCol] in ['R','B']:
            lst.append(jmp)
    return lst

def finish(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc):
    finishList=[]
    moves=movesIntoSafeSpot(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    goal=countRow(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    for move in moves:
        if ord(move[0])-65>goal:
            if (ord(move[3])-65)-(ord(move[0])-65)<0:
                finishList.append(move)
        else:
            if (ord(move[3])-65)-(ord(move[0])-65)>0:
                finishList.append(move)
    return finishList
...
def getMove(board,playerIndex,playerColors,playerTokens,opponentTokens,rowInc):
    ...
    jumpsFromPossibleJumpsList=jumpsFromPossibleJump(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    jumpsIntoSideSquareList=jumpsIntoSideSquare(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    jumpsBlockingJumpsList=jumpBlockJump(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    jumpsKingCheckerList=jumpKingChecker(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    ...
    movesIntoSideSquareList=moveIntoSideSquare(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    movesBlockingJumpsList=moveBlockJump(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    movesFromPossibleJumpsList=moveFromJump(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
    if len(validJumps)==0:
        if countEnemyCheckers(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)<5 and movesIntoSafeSpotList!=[] and finish(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)!=[]:
            moves=finish(validMoves,validJumps,board,playerIndex,playerTokens,opponentTokens,rowInc)
        ...
        elif movesFromPossibleJumpsList != []:
            move= movesFromPossibleJumpsList[random.randrange(0,len(movesFromPossibleJumpsList))]
        elif movesBlockingJumpsList != []:
            move=movesBlockingJumpsList[random.randrange(0,len(movesBlockingJumpsList))]
        ...
        elif jumpsFromPossibleJumpsList !=[]:
            move=jumpsFromPossibleJumpsList[random.randrange(0,len(jumpsFromPossibleJumpsList))]
        elif jumpsBlockingJumpsList != []:
            move=jumpsBlockingJumpsList[random.randrange(0,len(jumpsBlockingJumpsList))]
        elif jumpsKingCheckerList != []:
            move=jumpsKingCheckerList[random.randrange(0,len(jumpsKingCheckerList))]
